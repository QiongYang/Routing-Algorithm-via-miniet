    ...
    for source in sws:
      for i in sws:
        for k in range (len(sws)):
          ...
          for i in sws:
            if I[i]==0:
              if distance[(source,i)]<shortest_dis:
                shortest_dis=distance[(source,i)]
                shortest_index=i
                U.append(i)
          for j in sws:
            if I[j]==0:
              sourcej_dis=shortest_dis+diatance[(shortest_index,j)]
              if sourcej_dis<diatance[(source,j)]:
                distance[(source,j)]=sourcej_dis
                path_map[source][j]=(sourcej_dis, shortest_index)
